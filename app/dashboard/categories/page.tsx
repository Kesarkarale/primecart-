...
export default function ProductsPage() {
  ...
  const [buyingProductId, setBuyingProductId] =
    useState<string | null>(null);
  ...
  useEffect(() => {
    async function loadProducts() {
      ...
      try {
        ...
        const highest = Math.max(1000, ...loadedProducts.map((p) => Number(p.price ?? 0)));
    ...
    const syncCart = () => updateCartCount();
    window.addEventListener("storage", syncCart);
    window.addEventListener("primecart-cart-updated", syncCart);
    return () => {
      window.removeEventListener("storage", syncCart);
      window.removeEventListener("primecart-cart-updated", syncCart);
    };
  }, [supabase]);
  ...
  */
  ...
  async function buyNow(product: Product) {
    ...
    try {
      ...
      window.dispatchEvent(new Event("primecart-cart-updated"));
      router.push(`/dashboard/checkout?buyNow=${product.id}`);
    } catch {
      showToast("Unable to start checkout.");
    } finally {
      window.setTimeout(() => setBuyingProductId(null), 600);
    }
  }
  ...
  */
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (flashOnly) {
      result = result.filter((product) => Boolean(product.is_flash_sale));
    }
  ...
  */
  ...
  const smartFilters = useMemo(() => {
    ...
    if (name.includes("mobile")) return ["Under ₹20,000", "5★ Picks", "Featured", "Flash Sale"];
    if (name.includes("footwear")) return ["4★+", "Under ₹2,500", "50%+ Off", "In Stock"];
    if (name.includes("watch")) return ["Premium Brands", "Under ₹5,000", "4★+", "Deals"];
    if (name.includes("fashion")) return ["50%+ Off", "Trending", "4★+", "In Stock"];
    if (name.includes("gaming")) return ["Featured", "4★+", "Flash Sale", "Under ₹25,000"];
    if (name.includes("appliance")) return ["Under ₹10,000", "4★+", "Energy Picks", "Deals"];
    if (name.includes("automotive")) return ["In Stock", "4★+", "Deals", "Featured"];
    if (name.includes("toy") || name.includes("baby")) return ["Under ₹2,000", "4★+", "Deals", "In Stock"];
    if (name.includes("bag")) return ["Under ₹3,000", "50%+ Off", "4★+", "Trending"];
    if (name.includes("home")) return ["Under ₹5,000", "4★+", "Deals", "Featured"];
    return ["Top Rated", "Biggest Discount", "Flash Sale", "In Stock"];
  }, [selectedCategoryName]);

  const suggestions = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return products.filter((p) => p.is_featured).slice(0, 5);
    return products.filter((p) => `${p.name} ${p.brand ?? ""} ${p.short_description ?? ""}`.toLowerCase().includes(q)).slice(0, 6);
  }, [products, search]);

  function applySmartFilter(label: string) {
    const lower = label.toLowerCase();
    if (lower.includes("flash")) setFlashOnly(true);
    else if (lower.includes("featured") || lower.includes("trending")) setFeaturedOnly(true);
    else if (lower.includes("stock")) setStockOnly(true);
    else if (lower.includes("50%")) setDiscountFilter(50);
    else if (lower.includes("30%")) setDiscountFilter(30);
    else if (lower.includes("4★") || lower.includes("5★")) setRatingFilter(lower.includes("5★") ? 5 : 4);
    else if (lower.includes("discount") || lower.includes("deals")) setSort("discount");
    else if (lower.includes("rated")) setSort("rating");
  ...
  return (
    <main className="min-h-screen bg-[#faf8f3] text-[#17130d]">
        ...
        <div className="mx-auto flex h-[72px] max-w-[1500px] items-center gap-4 px-5 sm:px-8 lg:px-10">
          ...
          <div className="mx-auto hidden max-w-2xl flex-1 md:block">
            <div className="relative">
              ...
              {searchFocused && (
                <div className="absolute left-0 right-0 top-14 z-50 overflow-hidden rounded-2xl border border-[#eadfc9] bg-white shadow-[0_20px_60px_rgba(60,45,20,.14)]">
                  <div className="border-b border-[#eee7da] px-4 py-3 text-[10px] font-black uppercase tracking-[.14em] text-[#967400]">
                    {search ? "Search suggestions" : "Popular picks"}
                  </div>
                  {suggestions.length ? suggestions.map((item) => (
                    <button key={item.id} onClick={() => { setSearch(item.name); setSearchFocused(false); }} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-[#fffaf0]">
                      <Search size={14} className="text-[#b9975b]" />
                      <span className="min-w-0 flex-1 truncate text-sm font-bold">{item.name}</span>
                      <span className="text-xs text-[#9a9184]">{formatPrice(item.price)}</span>
                    </button>
                  )) : <p className="px-4 py-5 text-xs text-gray-500">No suggestions found.</p>}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-7 pb-24 sm:px-8 lg:px-10 lg:py-10 lg:pb-10">
        {/* BREADCRUMB */}
        ...
        <section className="mb-7 rounded-2xl border border-[#eadfc9] bg-white p-4 sm:p-5">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[.15em] text-[#967400]">Smart filters</p>
              <p className="mt-1 text-xs text-[#8d8477]">Quick picks for {selectedCategoryName}</p>
            </div>
            <Flame size={17} className="text-[#b9975b]" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {smartFilters.map((label) => (
              <button key={label} onClick={() => applySmartFilter(label)} className="shrink-0 rounded-full border border-[#eadfc9] bg-[#fffaf0] px-3.5 py-2 text-[11px] font-black text-[#765c1a] transition hover:-translate-y-0.5 hover:border-[#c79a3b] hover:bg-[#fff3cd]">
                {label}
              </button>
            ))}
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-[#eadfc9] bg-white/95 p-2 shadow-[0_-12px_40px_rgba(40,30,10,.10)] backdrop-blur md:hidden">
        <div className="mx-auto grid max-w-xl grid-cols-2 gap-2">
          <button onClick={() => setMobileFilters(true)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#17130d] text-xs font-black text-white"><SlidersHorizontal size={15}/> Filter{activeFilterCount ? ` (${activeFilterCount})` : ""}</button>
          <div className="relative">
            <select value={sort} onChange={(e)=>setSort(e.target.value)} className="h-11 w-full appearance-none rounded-xl border border-[#eadfc9] bg-white px-4 text-xs font-black"><option value="featured">Recommended</option><option value="rating">Top Rated</option><option value="discount">Biggest Discount</option><option value="price-low">Price Low</option><option value="price-high">Price High</option></select>
      ...
      <div className="space-y-6">
        ...
        <FilterSection title="Product Type">
          <div className="space-y-2">
            <label className="flex cursor-pointer items-center justify-between rounded-lg bg-[#faf8f3] px-3 py-2.5 text-xs font-bold">
              Flash Sale
              <input type="checkbox" checked={flashOnly} onChange={(e) => setFlashOnly(e.target.checked)} className="h-4 w-4 accent-[#b9975b]" />
            </label>
            <label className="flex cursor-pointer items-center justify-between rounded-lg bg-[#faf8f3] px-3 py-2.5 text-xs font-bold">
              Featured
              <input type="checkbox" checked={featuredOnly} onChange={(e) => setFeaturedOnly(e.target.checked)} className="h-4 w-4 accent-[#b9975b]" />
            </label>
          </div>
        </FilterSection>
...
function ProductCard({
  ...
  return (
    <article
      ...
      onKeyDown={(event) => {
        if (event.key === "Enter") router.push(`/dashboard/products/${product.id}`);
      }}
      role="link"
      tabIndex={0}
      className="group cursor-pointer overflow-hidden rounded-[25px] border border-[#eadfc9] bg-white shadow-[0_7px_28px_rgba(60,45,20,.035)] transition duration-500 hover:-translate-y-2 hover:border-[#d7c08b] hover:shadow-[0_25px_60px_rgba(60,45,20,.13)]"
      style={{
        ...
        {image && !imageError ? (
          <Image
            ...
            onError={(event) => {
              ...
              if (filename && current.includes(`/products/${filename}`)) {
                target.src = `/${filename}`;
                return;
              }
              setImageError(true);
      ...
      <div className="p-5">
        ...
        <div className="mt-4 grid grid-cols-[1fr_1fr] gap-2">
          <button
            onClick={onAdd}
            disabled={stock <= 0 || isAdding}
            className={`flex h-11 items-center justify-center gap-2 rounded-xl text-xs font-black ${stock <= 0 ? "bg-[#eeeae2] text-gray-400" : "bg-[#17130d] text-white hover:bg-[#b9975b]"}`}
          >
            {isAdding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" /> : <><ShoppingBag size={15} /> Add</>}
          </button>
          <button
            onClick={onBuyNow}
            disabled={stock <= 0 || isBuying}
            className={`flex h-11 items-center justify-center gap-2 rounded-xl border text-xs font-black ${stock <= 0 ? "border-[#eadfc9] bg-[#f3f0ea] text-gray-400" : "border-[#b9975b] bg-[#fff8e5] text-[#7e6000] hover:bg-[#ffeeb5]"}`}
          >
            {isBuying ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-[#b9975b]/30 border-t-[#b9975b]" /> : <><Zap size={15} /> Buy Now</>}
          </button>
        </div>
      </div>
    </article>
        ...
        >
          {image && !imageError ? (
            <Image
              ...
              onError={(event) => {
                const filename = image?.split("/").pop();
                if (filename && event.currentTarget.src.includes(`/products/${filename}`)) {
                  event.currentTarget.src = `/${filename}`;
                  return;
                }
                setImageError(true);
              }}
...
function CompareModal({ products, compareIds, categories, onClose, onRemove }: {
  products: Product[];
  compareIds: string[];
  categories: Category[];
  onClose: () => void;
  ...
  const selected = compareIds.map((id) => products.find((p) => p.id === id)).filter((p): p is Product => Boolean(p));
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/55 backdrop-blur-sm" onClick={onClose} />
      <div className="relative max-h-[90vh] w-full max-w-6xl overflow-auto rounded-[28px] bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#eadfc9] bg-white px-5 py-4 sm:px-7">
          <div><h2 className="text-xl font-black">Compare Products</h2><p className="text-xs text-gray-500">Compare price, rating, discount and stock.</p></div>
          <button onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f7f2e8]"><X size={18}/></button>
        </div>
        <div className="min-w-[680px] p-5 sm:p-7">
          <div className="grid gap-3" style={{gridTemplateColumns:`180px repeat(${Math.max(selected.length,1)}, minmax(190px,1fr))`}}>
            <div />
            {selected.map((p)=><div key={p.id} className="relative rounded-2xl border border-[#eadfc9] bg-[#faf8f3] p-4">
              <button onClick={()=>onRemove(p.id)} className="absolute right-2 top-2 text-gray-400 hover:text-red-500"><X size={14}/></button>
              <div className="relative mx-auto h-32 w-full rounded-xl bg-white"><Image src={getImageUrl(p.image_url) || "/logo.png"} alt={p.name} fill sizes="200px" className="object-contain p-3"/></div>
              <p className="mt-3 line-clamp-2 text-sm font-black">{p.name}</p>
            </div>)}
            {[
              ["Category", (p:Product)=>getCategoryName(p,categories)],
              ["Brand", (p:Product)=>p.brand || "—"],
              ["Price", (p:Product)=>formatPrice(p.price)],
              ["Rating", (p:Product)=>`${Number(p.rating??0).toFixed(1)} ★ (${p.reviews_count??0})`],
              ["Discount", (p:Product)=>`${getDiscount(Number(p.price??0),Number(p.original_price??0))}%`],
              ["Stock", (p:Product)=>Number(p.stock??0)>0?`${p.stock} available`:"Out of stock"],
              ["Flash Sale", (p:Product)=>p.is_flash_sale?"Yes":"No"],
              ["Featured", (p:Product)=>p.is_featured?"Yes":"No"],
            ].map(([label,get])=><div key={String(label)} className="contents">
              <div className="border-t border-[#eee7da] bg-white px-3 py-3 text-xs font-black text-[#756b5c]">{String(label)}</div>
              {selected.map((p)=><div key={p.id+String(label)} className="border-t border-[#eee7da] px-3 py-3 text-xs font-bold">{(get as (p:Product)=>string)(p)}</div>)}
            </div>)}
          </div>
        </div>
      </div>
...
function FlashCountdown() {
  ...
  useEffect(() => {
    const get = () => { const now = new Date(); const end = new Date(now); end.setHours(now.getHours()+5,59,59,999); return Math.max(0,end.getTime()-Date.now()); };
    setRemaining(get());
    const id=window.setInterval(()=>setRemaining(get()),1000);
    return ()=>window.clearInterval(id);
  }, []);
  const total=Math.floor(remaining/1000); const h=Math.floor(total/3600); const m=Math.floor((total%3600)/60); const sec=total%60;
  return <div className="flex items-center gap-2 rounded-full bg-[#fff5d4] px-3 py-2 text-[10px] font-black text-[#7e6000]"><Clock3 size={13}/>{String(h).padStart(2,"0")}:{String(m).padStart(2,"0")}:{String(sec).padStart(2,"0")}</div>;
}
