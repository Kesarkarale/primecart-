...
export default function OrderSuccessClient() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (lastOrder) {
        loadedOrder = JSON.parse(lastOrder);
      }

      if (!loadedOrder) {
        const ordersString = localStorage.getItem("primecart-orders");

        if (ordersString) {
          const orders = JSON.parse(ordersString);

          if (Array.isArray(orders) && orders.length > 0) {
            loadedOrder = orders[orders.length - 1];
          }
        }
      }
    } catch (error) {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#fffdf9]">
        <div className="flex flex-col items-center gap-4">
          <div className="h-12 w-12 animate-spin rounded-full border-4 border-[#ead9b7] border-t-[#b8872d]" />
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#fffdf9] text-[#2b241b]">
